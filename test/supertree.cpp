...
TEST_CASE("check_supertree1", "[supertree]") {
	constraints c = {};
	CHECK(check_supertree(std::vector<index>{0, 1}, c) == false);
}

TEST_CASE("check_supertree2", "[supertree]") {
	constraints c = {};
	CHECK(check_supertree(std::vector<index>{0, 1, 2}, c) == true);
}

TEST_CASE("check_supertree3", "[supertree]") {
	constraints c = {};
	CHECK(check_supertree(std::vector<index>{0, 1, 2, 3, 4, 5, 6}, c) == true);
}

TEST_CASE("check_supertree4", "[supertree]") {
	constraints c = {{0, 1, 2}};
	CHECK(check_supertree(std::vector<index>{0, 1, 2}, c) == false);
}

TEST_CASE("check_supertree5", "[supertree]") {
	constraints c = {{0, 1, 2}, {2, 3, 4}}; // not really a valid test
	CHECK(check_supertree(std::vector<index>{0, 1, 2, 3}, c) == false);
}

TEST_CASE("check_supertree6", "[supertree]") {
	constraints c = {{1, 0, 2}, {3, 4, 1}};
	CHECK(check_supertree(std::vector<index>{0, 1, 2, 3, 4}, c) == true);
}

TEST_CASE("count_supertree1", "[supertree]") {
	constraints c = {};
	CHECK(count_supertree(std::vector<index>{0, 1}, c) == 1);
}

TEST_CASE("count_supertree2", "[supertree]") {
	constraints c = {};
	CHECK(count_supertree(std::vector<index>{0, 1, 2}, c) == 3);
}

TEST_CASE("count_supertree3", "[supertree]") {
	constraints c = {};
	CHECK(count_supertree(std::vector<index>{0, 1, 2, 3, 4, 5, 6}, c) == 10395);
}

TEST_CASE("count_supertree4", "[supertree]") {
	constraints c = {{0, 1, 2}};
	CHECK(count_supertree(std::vector<index>{0, 1, 2}, c) == 1);
}

TEST_CASE("count_supertree5", "[supertree]") {
	constraints c = {{0, 1, 2}, {2, 3, 4}};
	CHECK(count_supertree(std::vector<index>{0, 1, 2, 3}, c) == 1);
}

TEST_CASE("count_supertree6", "[supertree]") {
	constraints c = {{1, 0, 2}, {3, 4, 1}};
	CHECK(count_supertree(std::vector<index>{0, 1, 2, 3, 4}, c) == 9);
}

TEST_CASE("count_supertree7", "[supertree]") {
	constraints c = {{0, 1, 3}, {3, 2, 0}, {4, 5, 6}, {6, 3, 4}, {2, 3, 6}, {2, 6, 7}};
	CHECK(count_supertree(std::vector<index>{0, 1, 2, 3, 4, 5, 6, 7}, c) == 173);
}
