...
TEST_CASE("constraints", "[constraints]") {
	...
	CHECK(apply_constraints(leaves1.size(), filter_constraints(leaves1, c)) == target_leaves1);
	CHECK(apply_constraints(leaves2.size(), filter_constraints(leaves2, c)) == target_leaves2);
}
