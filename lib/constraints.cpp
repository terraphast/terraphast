...
constraints filter_constraints(std::vector<index> leaves, constraints c) {
        ...
        for (size_t i = 0; i < c.size(); i++) {
                if ((std::find(leaves.begin(), leaves.end(), c.at(i).shared) != leaves.end()) && (std::find(leaves.begin(), leaves.end(), c.at(i).left) != leaves.end())) {
...
std::vector<std::vector<index>> apply_constraints(std::vector<index> species,
                                                  constraints c) {
